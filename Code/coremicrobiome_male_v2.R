...
phyloseq_male_RA <- transform_sample_counts(deer_male, fun=function(x) x/sum(x))
...
male_neg_ASVs <- core_members(male_neg, detection = 0.001, prevalence = 0.2)
male_ln_pos_ASVs <- core_members(male_ln_pos, detection = 0.001, prevalence = 0.2)
male_br_ln_pos_ASVs <- core_members(male_br_ln_pos, detection = 0.001, prevalence = 0.2)
